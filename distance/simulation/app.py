# simulation/app.py
import os, sys, time, tkinter as tk, traci, math
...
def wait_until_all_parked(traci_mod, timeout=180.0):
    ...
    while time.time() - t0 < timeout:
        ...
        # 차량이 하나도 없으면 계속 시뮬레이션 진행
        if not ids:
            if step_count % 100 == 0:
                print(f"[INFO] 차량 대기 중... (시뮬레이션 시간: {traci_mod.simulation.getTime():.1f}s)")
            continue
        ...
        for vid in ids:
            try:
                if not traci_mod.vehicle.isStopped(vid):
                    all_stopped = False
                    break
        ...
        if all_stopped:
            print(f"[INFO] 모든 차량 주차 완료. (시뮬레이션 시간: {traci_mod.simulation.getTime():.1f}s)")
            return True
        
        # 진행 상황 출력
        if step_count % 200 == 0:
            moving_count = sum(1 for vid in ids if not traci_mod.vehicle.isStopped(vid))
            print(f"[INFO] 주차 대기 중... (이동 중: {moving_count}/{len(ids)})")
...
def run():
    ...
    def update_loop():
        ...
        # release_index가 현재 체인 길이를 초과하지 않도록 조정
        if release_index >= len(current_chain):
            release_index = len(current_chain) - 1
        
        if release_index >= 0 and release_index < len(current_chain):
            # 현재 체인 기준으로 출발 대상 확인
            vid = current_chain[release_index]
            
            # ready_to_release_next 함수가 현재 체인을 참조하도록 수정
            def ready_to_release_next():
                ...
                # 앞차 조건 확인
                if release_index <= 0 or release_index >= len(current_chain):
                    return False
                    
                prev_id = current_chain[release_index - 1]
                try:
                    # 앞차의 현재 엣지/엣지 내 위치/속도
                    road     = traci.vehicle.getRoadID(prev_id)
                    lane_pos = traci.vehicle.getLanePosition(prev_id)
                    v        = traci.vehicle.getSpeed(prev_id)
            ...
            if ready_to_release_next():
                try:
                    # 차량이 존재하는지 확인
                    if vid not in traci.vehicle.getIDList():
                        release_index += 1  # 차량이 없으면 다음으로
                    else:
                        # 주차장에 있는지 확인
                        lane_id = traci.vehicle.getLaneID(vid)
                        road_id = traci.vehicle.getRoadID(vid)
                        is_in_parking = (not lane_id or lane_id.startswith("pa_") or 
                                        road_id.startswith("pa_"))
                        
                        if is_in_parking and traci.vehicle.isStopped(vid):
                            # 멈춤 상태 해제
                            traci.vehicle.resume(vid)
                            # stop 명령이 있으면 제거
                            try:
                                traci.vehicle.stop(vid, duration=0, flags=0)
                            except:
                                pass
                            print(f"[START] {vid} 출발 (주차장에서)")
                            released.append(vid)
                            started_vehicles.add(vid)  # 출발한 차량 기록
        ...
        # 계기판 컬럼 재배치 (모든 차량)
        all_existing_vehicles_list = list(traci.vehicle.getIDList())
        for idx, vid in enumerate(all_existing_vehicles_list):
            if vid in meters:
                try:
                    canv2, needle2, lab2 = meters[vid]
                    canv2.grid(row=0, column=idx)
                    lab2.grid(row=1, column=idx)
                except:
                    pass
        ...
        # 플래튜닝 차량의 차선 변경 금지
        from simulation.platoon import enforce_no_lane_change
        enforce_no_lane_change()
