...
class VehicleViewer(tk.Toplevel):
    ...
    def __init__(self, parent, traci_mod, initial_candidates):
        ...
        self.listbox = tk.Listbox(self.right, width=22, height=18)
    ...
    def _refresh_now(self):
        try:
            ...
            if me in chain:
                ...
                if not cand:
                    self.listbox.insert(tk.END, "— 300m 내 참여 후보 없음 —")
                else:
                    # cand: [(platoon_vid, dist), ...] (app.py에서 이미 거리 오름차순 정렬됨)
                    tail = chain[-1] if chain else None
                    for v, d in cand:
                        mark = " (tail)" if v == tail else ""
                        self.listbox.insert(tk.END, f"{v} : {d:.1f} m{mark}")
                # 후보 리스트는 하이라이트 없음(선택 차량은 미참여이므로)
