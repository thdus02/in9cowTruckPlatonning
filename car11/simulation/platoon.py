...
def control_follower_speed(follower_id, leader_id):
    try:
        ...
        # 0) 플래투닝 차량이 재참여 중인지 확인 (JOIN_COOLDOWN 체크)
        try:
            ...
            if leader_id in JOIN_COOLDOWN:
                ...
                if sim_t < until_t:
                    # 리더가 재참여 중이면 뒷팔로워가 확실히 감속 (최소 3m/s 차이)
                    try:
                        vL = traci.vehicle.getSpeed(leader_id)
        ...
        is_cutin_approaching = pair_key in cfg.CUT_IN_ACTIVE_PAIRS
        base_ref_gap = STANDSTILL_GAP + TIME_HEADWAY * max(vF, 0.0)

        if gap_m is None:
            # 앞차를 못 찾는 경우 현재속도보다 감속
            traci.vehicle.setSpeed(follower_id, max(0.0, vF - 1.5))
            return

        if is_cutin_approaching:
            leader_info = traci.vehicle.getLeader(follower_id, 200.0)
            if leader_info and leader_info[0] == leader_id:
                platoon_gap = float(leader_info[1])
                target_gap = CUT_IN_EXPAND_GAP
                err = platoon_gap - target_gap
                if err < 0:
                    gap_error = abs(err)
                    decel_factor = min(4.0, gap_error * 0.2)
                    v_cmd = max(8.0, vF - decel_factor)
                    try:
                        vL = traci.vehicle.getSpeed(leader_id)
                        v_cmd = min(v_cmd, vL - 2.0)
                    except:
                        pass
                    v_cmd = max(8.0, min(v_cmd, V_MAX_FOLLOW))
                    traci.vehicle.setSpeed(follower_id, v_cmd)
                    return
                elif err > 5.0:
