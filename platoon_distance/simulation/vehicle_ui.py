...
class VehicleViewer(tk.Toplevel):
    ...
    def _refresh_buttons(self):
        """내 차량이 체인에 있는지 여부와 거리에 따라 버튼 활성화 상태 토글"""
        me = self.selected.get()
        chain = _order_chain(cfg.FOLLOW_PAIRS)
        in_platoon = me in chain

        # 플래튜닝에 참여하지 않은 경우, 거리 확인
        if not in_platoon:
            # 거리 확인 (300m 이내인지)
            distance = cfg.VEHICLE_DISTANCES.get(me, float('inf'))
            from simulation.app import PLATOON_JOIN_DISTANCE
            within_range = distance <= PLATOON_JOIN_DISTANCE
            
            # 300m 이내이고 플래튜닝에 참여하지 않은 경우에만 활성화
            self.btn_join.configure(state=("normal" if within_range else "disabled"))
        else:
            # 이미 플래튜닝에 참여 중이면 비활성화
            self.btn_join.configure(state="disabled")
        
        # ui에서 선택 안되고, 출발 안 했을 때만 버튼 활성화
        can_start = (me not in chain) and (me not in cfg.STARTED)
