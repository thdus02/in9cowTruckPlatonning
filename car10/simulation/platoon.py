...
def control_follower_speed(follower_id, leader_id):
    try:
        ...
        # 3) 끼어들기 접근 중인지 확인하여 목표 간격 결정 (CACC/ACC 스타일)
        pair_key = (leader_id, follower_id)
        is_cutin_approaching = pair_key in CUT_IN_ACTIVE_PAIRS

        # === CACC 스타일 목표 간격 ===
        # 기본: d_ref = STANDSTILL_GAP + TIME_HEADWAY * vF
        base_ref_gap = STANDSTILL_GAP + TIME_HEADWAY * max(vF, 0.0)

        # 끼어들기 접근 중이면 여유 margin 추가
        # (기존 CUT_IN_EXPAND_GAP와 DESIRED_GAP의 차이만큼 더 띄워줌)
        if is_cutin_approaching:
            extra_margin = max(0.0, CUT_IN_EXPAND_GAP - DESIRED_GAP)
            target_gap = base_ref_gap + extra_margin
        else:
            target_gap = base_ref_gap

        # === CACC 제어식 ===
        # a_cmd = aL + Kp * (d - d_ref) + Kd * (vL - vF)
        err  = gap_m - target_gap       # 거리 오차
        vrel = vT - vF                  # 상대속도 (리더 - 팔로워)

        # 리더(실제 앞차)의 가속도 피드포워드
        try:
