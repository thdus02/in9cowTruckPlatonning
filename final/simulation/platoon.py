...
# -----------------------------------------------------------------------------
# ③ 정상 추종 제어 (매 스텝 호출)
#    끼어든 차량 포함 '실제 앞차'를 타겟으로 TIME_HEADWAY 기준 유지 (PD + catch-up)
_KP = 0.8
_KD = 0.4
...
def control_follower_speed(follower_id, leader_id):
    try:
        ...
        # -----------------------------------------------------------------
        # [0] 재합류 쿨다운(JOIN_COOLDOWN) 보호 - 예전 버전에서 사용하던 로직
        #     리더가 재합류 쿨다운 중이면 바로 뒤 팔로워가 확실히 감속
        # -----------------------------------------------------------------
        try:
            from simulation.vehicle_ui import JOIN_COOLDOWN
            sim_t = traci.simulation.getTime()
            if leader_id in JOIN_COOLDOWN:
                until_t = JOIN_COOLDOWN.get(leader_id, 0)
                if sim_t < until_t:
                    try:
                        vL = traci.vehicle.getSpeed(leader_id)
                        vF = traci.vehicle.getSpeed(follower_id)
                        target_speed = max(5.0, vL - 3.0)
                        if vF > target_speed + 0.2:
                            traci.vehicle.setSpeed(follower_id, target_speed)
                            return  # 쿨다운 중에는 여기서 끝
                    except:
                        pass
        except:
            pass
        ...
        # -----------------------------------------------------------------
        # [1] 합류 지원 (Scenario 1)
        #     리더와 다른 차선일 때, 너무 가까우면 살짝 감속해서 갭 확보.
        #     단, CUT_IN_ACTIVE_PAIRS 중일 땐 여기서 또 브레이크 안 밟게 차단.
        # -----------------------------------------------------------------
        try:
            # 끼어들기 플래그가 켜져 있으면 Scenario 1은 스킵
            if pair_key not in CUT_IN_ACTIVE_PAIRS and leader_id in traci.vehicle.getIDList():
                f_lane = traci.vehicle.getLaneID(follower_id)
                l_lane = traci.vehicle.getLaneID(leader_id)

                # 리더와 내가 다른 차선(= 합류 대기 중)
                if f_lane and l_lane and (f_lane != l_lane):
                    # 유클리드 거리 직접 계산 (getLeader에 의존 X)
                    f_pos = traci.vehicle.getPosition(follower_id)
                    l_pos = traci.vehicle.getPosition(leader_id)
                    dx = f_pos[0] - l_pos[0]
                    dy = f_pos[1] - l_pos[1]
                    dist_direct = (dx * dx + dy * dy) ** 0.5

                    MERGE_GAP_REQUIRED = 35.0  # 합류를 위해 확보해야 할 거리(약간 완화)

                    if dist_direct < MERGE_GAP_REQUIRED:
                        vL = traci.vehicle.getSpeed(leader_id)

                        # 거리 좁을수록 brake_force 증가하지만, 너무 세게는 X
                        gap_diff = MERGE_GAP_REQUIRED - dist_direct
                        brake_force = 0.5 + (gap_diff * 0.05)   # 예: 최대 2~3 정도

                        # 기본 목표: 리더보다 약간 느리고, 내 현재 속도에서 부드럽게 감속
                        target_v = vF - brake_force

                        # 한 번에 너무 세게 줄이지 않기 (최대 -3 m/s)
                        target_v = max(target_v, vF - 3.0)

                        # 리더보다 너무 느리게 가지 않도록 (리더 - 3.0 이상)
                        target_v = max(target_v, vL - 3.0)

                        # 앞에 물리적인 차(A)가 있으면 그 속도도 고려
                        try:
                            info = traci.vehicle.getLeader(follower_id, 100.0)
                            if info:
                                phys_leader, phys_gap = info
                                v_phys = traci.vehicle.getSpeed(phys_leader)
                                if phys_gap < 20.0:
                                    # 실제 바로 앞차가 더 느리면 그 차 속도에 맞추도록
                                    target_v = min(target_v, v_phys - 1.0)
                        except:
                            pass

                        target_v = max(0.0, target_v)
                        traci.vehicle.setSpeed(follower_id, target_v)
                        return  # 합류 브레이크 중에는 CACC 가속 로직 차단
        except Exception:
            pass

                # -----------------------------------------------------------------
        # [2] 일반 끼어들기(Cut-In) 양보 (Scenario 2) - CUT_IN_ACTIVE_PAIRS 기반
        # -----------------------------------------------------------------
        pair_key = (leader_id, follower_id)
        if pair_key in CUT_IN_ACTIVE_PAIRS:
            # config 값과 통일
            YIELD_TARGET_GAP = float(CUT_IN_EXPAND_GAP)
            MARGIN = 5.0  # 목표 ±5m 안쪽이면 더 이상 강제 제어 X

            target_id, gap_m = _pick_front_target(follower_id, leader_id, lookahead=250.0)

            # gap 정보를 못 받으면 그냥 CACC에 맡김
            if gap_m is None:
                return

            # ① 아직 너무 좁음 → 살짝 감속해서 공간 만들어주기
            if gap_m < YIELD_TARGET_GAP - MARGIN:
                gap_err = YIELD_TARGET_GAP - gap_m
                # 부드러운 감속 (이전보다 많이 약함)
                decel = max(0.3, gap_err * 0.08)   # 0.3 ~ 2.x 정도
                decel = min(decel, 2.5)

                v_yield = vF - decel

                if target_id in traci.vehicle.getIDList():
                    vT = traci.vehicle.getSpeed(target_id)
                    if vT < vF:
                        v_yield = min(v_yield, vT - 1.0)

                # 한 번에 너무 많이 안 줄이기
                v_yield = max(v_yield, vF - 3.0)
                v_yield = max(0.0, v_yield)
                traci.vehicle.setSpeed(follower_id, v_yield)
                return

            # ② 이미 충분히 벌어짐 → 너무 멀어지면 살짝 가속
            if gap_m > YIELD_TARGET_GAP + MARGIN:
                # 목표보다 5m 이상 크게 벌어지면 조금씩 리더 쪽으로 다가가기
                if target_id in traci.vehicle.getIDList():
                    vT = traci.vehicle.getSpeed(target_id)
                else:
                    vT = vF

                # 앞차보다 살짝 빠르게, 하지만 상한은 V_MAX_FOLLOW
                v_hold = min(vT + 1.5, V_MAX_FOLLOW)
                # 너무 급가속은 금지 (한 번에 +2m/s 이상 안 올라가게)
                v_hold = min(v_hold, vF + 2.0)

                traci.vehicle.setSpeed(follower_id, max(0.0, v_hold))
                return

            # ③ YIELD_TARGET_GAP ±5m 안 → 갭 좋은 상태, 여기서는 별도 강제 제어 X
            #    아래 CACC(Scenario 3)로 넘겨서 자연스럽게 유지하게 둠
            #    즉, "감속 관두고, 가속/유지는 PD 제어에 맡김" 느낌

        # -----------------------------------------------------------------
        # [3] 일반 주행 (Normal CACC) - PD + Catch-up
        # -----------------------------------------------------------------
        target_id, gap_m = _pick_front_target(follower_id, leader_id, lookahead=250.0)
        vT = traci.vehicle.getSpeed(target_id) if target_id in traci.vehicle.getIDList() else vF

        # 앞차를 못 찾는 경우 → 보수적으로 감속
        if gap_m is None:
            v_cmd = min(vT + 1.5, V_MAX_FOLLOW) if vT > 0 else max(0.0, vF - 1.5)
            traci.vehicle.setSpeed(follower_id, v_cmd)
            return

        # CACC 기준 간격
        base_ref_gap = STANDSTILL_GAP + TIME_HEADWAY * max(vF, 0.0)
        target_gap = base_ref_gap
        ...
        try:
            if target_id in traci.vehicle.getIDList():
                aL = traci.vehicle.getAcceleration(target_id)
            else:
                aL = 0.0
        except:
            aL = 0.0

        a_cmd = aL + _KP * err + _KD * vrel
        v_cmd = vF + a_cmd * _DT

        # --- Catch-up: 너무 멀어지면 상한 풀고 보너스 추격 (예전 로직 유지) ---
        if err > 10.0:
            try:
        ...
        # 그 외: 상한만 V_MAX_FOLLOW
        else:
            v_cmd = max(0.0, min(v_cmd, V_MAX_FOLLOW))

        # 너무 가까우면 앞차보다 확실히 느리게 (새 버전 쪽 기준 조금 강화)
        if err < -2.0:
            v_cmd = min(v_cmd, vT - 1.0)
    ...
    try:
        ...
        for vid in traci.simulation.getDepartedIDList():
            if vid in current_followers and vid not in _boosted:
                ...
                try:
                    traci.vehicle.setMaxSpeed(vid, V_MAX_FOLLOW)  # 충분한 상한
...
# -----------------------------------------------------------------------------
##### 현재 사용 안 되고 있는 예전 코드(참고용). 필요하면 별도 실험용으로만 사용. #####
def expand_gap_for_cutin(leader_id: str, follower_id: str, cutin_car_id: str = None):
    """
    (레거시) 일반 차량이 끼어들려고 접근할 때 플래투닝 차량이 거리를 벌려주는 함수.
    지금은 control_follower_speed / CUT_IN_ACTIVE_PAIRS 기반 로직을 사용 중이라
    이 함수는 직접 호출하지 않는 것을 권장.
    """
    try:
        ...
        if cutin_car_id and cutin_car_id in traci.vehicle.getIDList():
            try:
                ...
                if dist_to_leader < CUT_IN_APPROACH_DISTANCE or dist_to_follower < CUT_IN_APPROACH_DISTANCE:
                    if dist_to_leader < dist_to_follower * 1.5:
                        should_expand = True
            except traci.exceptions.TraCIException:
                pass
        else:
...
def control_follower_speed_with_cutin_expansion(follower_id, leader_id, expanded_gap_target=None):
    """
    (레거시) 끼어들기 접근 시 확장된 간격을 유지하도록 하는 제어 함수.
    지금은 control_follower_speed() 쪽에서 통합 제어 중.
    """
    try:
        if follower_id not in traci.vehicle.getIDList():
            return
