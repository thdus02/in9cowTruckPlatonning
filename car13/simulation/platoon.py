...
def control_follower_speed(follower_id, leader_id):
    try:
        ...
        # -----------------------------------------------------------------
        # [긴급 수정] 재합류(Merge) 지원 로직 (유클리드 거리 기반)
        # 리더(B)가 옆 차선에 있으면, A(물리적 앞차)가 있든 없든 거리를 계산해서 감속한다.
        # -----------------------------------------------------------------
        try:
            if leader_id in traci.vehicle.getIDList():
                ...
                # 1. 리더와 내가 다른 차선(엣지)에 있음 (= 합류 대기 중)
                if f_lane and l_lane and (f_lane != l_lane):
                    ...
                    # 합류 안전 거리 (40m)
                    MERGE_GAP_REQUIRED = 40.0
                    
                    # 2. 리더가 40m 이내에 있다면 무조건 감속 (공간 만들기)
                    if dist_direct < MERGE_GAP_REQUIRED:
                        # 리더 속도
                        vL = traci.vehicle.getSpeed(leader_id)
                        
                        # 목표: 리더보다 확실히 느리게 가서 뒤로 물러나기
                        # 거리가 좁을수록 더 강력하게 브레이크
                        gap_diff = MERGE_GAP_REQUIRED - dist_direct
                        brake_force = 1.5 + (gap_diff * 0.2) # 기본 1.5 + 거리비례
                        
                        # 목표 속도: 리더 속도 - 3.0m/s (확실한 감속)
                        target_v = max(0.0, vL - 3.0)
                        
                        # 현재 내 속도에서 급감속 허용
                        if vF > target_v:
                            target_v = max(0.0, vF - brake_force)

                        # ★ 물리적 앞차(A) 충돌 방지 (혹시 A가 있으면 그것보단 느리게)
                        try:
        ...
        if err > 10.0:
            try:
                ...
                pass
            catch_bonus = min(5.0, err * 0.15) 
            v_cmd = max(v_cmd, vT + catch_bonus)
            v_cmd = max(0.0, min(v_cmd, 40.0))
...
# -----------------------------------------------------------------------------
#####현재 사용 안되고 있음 예전코드인듯. but 지금 끼어들기할때 간격 벌려주는거 잘 못함. #################
