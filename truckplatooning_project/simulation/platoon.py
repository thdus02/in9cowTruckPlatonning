import math
import traci
from .config import DESIRED_GAP, CATCH_GAIN, BRAKE_GAIN, V_MAX_FOLLOW, FOLLOW_PAIRS, FOLLOWERS

# --- 전역 상태(팔로워별 초기 락) ---
startup_lock_done  = {fid: False for fid in FOLLOWERS}
startup_lock_until = {fid: 0.0   for fid in FOLLOWERS}
...
def ensure_initial_gap_lock(follower_id, leader_id):
    ...
    if startup_lock_done.get(follower_id, False): return
    ...
    try:
        ...
        startup_lock_until[follower_id] = sim_t + 1.0
...
def control_follower_speed(follower_id, leader_id):
    ...
    ensure_no_overtake(follower_id); ensure_no_overtake(leader_id)
    vL = traci.vehicle.getSpeed(leader_id)
...
def boost_followers_once():
    ...
    for vid in traci.simulation.getDepartedIDList():
        if vid in FOLLOWERS:
            try:
                traci.vehicle.setMaxSpeed(vid, 40.0)  # 테스트용 넉넉히
            except traci.exceptions.TraCIException:
                pass
